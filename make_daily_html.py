...
输入：run_daily.py 评分后的 DataFrame（含 综合分/次日连板概率/评级/预测理由 等列）
输出：独立 HTML 文件，含统计卡、Top10 预测卡、全表（含预测理由）、ECharts 图。
"""
import json
...
def make_daily_html(df: pd.DataFrame, trade_date: str,
    ...
    for i, r in enumerate(top10.itertuples(), 1):
        reason = str(getattr(r, "预测理由", "") or "")
        top10_html.append(
            f"<tr><td>{i}</td><td>{getattr(r, '代码', '')}</td><td><b>{getattr(r, '名称', '')}</b></td>"
            f"<td>{int(getattr(r, '连板数', 1))}板</td><td>{getattr(r, '所属行业', '')}</td>"
            f"<td>{getattr(r, '综合分', 0):.1f}</td>"
            f"<td><b>{getattr(r, '次日连板概率', 0)*100:.1f}%</b></td>"
            f"<td><span class='rate rate-{str(getattr(r, '评级', 'D'))[:1].lower()}'>{getattr(r, '评级', '')}</span></td>"
...
<style>
  ...
  td.reason {{ text-align: left; font-size: 12px; color: #4e5969; min-width: 240px; }}
  ...
  .tbl {{ max-height: 560px; overflow: auto; }}
...
<div class="wrap">
<h1>首板连板概率模型 · 每日预测</h1>
<div class="sub">交易日 {trade_date} ｜ 依据当日涨停池 v2 评分（10 因子） ｜ 预测次日能否继续连板（尾盘买入口径） ｜ 每只股票附预测理由</div>
...
<div class="panel">
<h2>今日 Top10 连板概率预测（含预测理由）</h2>
<div class="grid2">
  <div class="chart" id="c_prob"></div>
  <div>
    <table>
      <tr><th>#</th><th>代码</th><th>名称</th><th>连板</th><th>题材</th><th>综合分</th><th>概率</th><th>评级</th><th>预测理由</th></tr>
      {top10_body}
    </table>
  </div>
</div>
...
<div class="tbl">
<table>
  <tr><th>#</th><th>代码</th><th>名称</th><th>题材</th><th>连板</th><th>首封</th><th>综合分</th><th>次日连板概率</th><th>评级</th><th>预测理由</th><th>最新价</th></tr>
{table_body}
</table>
</div>
</div>
...
<div class="note">
<b>模型口径（v2）：</b>连板身位 25% / 涨停频率(近10日涨停次数) 15% / 5日动量 15% / 市场情绪 10% / 个股身位 10% / 封板质量 10% / 封板时间 8% / 板块梯队 7%，再按连板身位基准晋级率校准为次日连板概率。<br>
历史回测（1 年，2025-06-02~2026-09-18，319 交易日，尾盘买入/次日连板口径）：全池 Top5 命中率 40.31%（基准 20.79%）；「Top1+身位≥6 精准模式」命中率 50.0%（胜率 67.1%）；首板专项 Top5 23.40% / Top1 25.08%（首板自然晋级率 16.4%）。<br>
本页为模型输出，非投资建议。
</div>
